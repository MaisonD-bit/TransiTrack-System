...
class CommuterRoutesController extends Controller
{
    ...
    public function bookTicket(Request $request)
    {
        $data = $request->validate([
            ...
            'from_stop_index' => ['nullable', 'integer', 'min:0'],
            'to_stop_index' => ['nullable', 'integer', 'min:0', 'gt:from_stop_index'],
        ]);
        ...
        if (! empty($data['schedule_id'])) {
            ...
            if (! $this->isScheduleWithinBookableWindow($schedule)) {
                return response()->json([
                    'success' => false,
        ...
        try {
            $ticket = Ticket::create([
                ...
                'to_stop_index' => $data['to_stop_index'] ?? null,
            ]);
        ...
        // Match by commuter_id first (strict), then fall back to email/name so a
        // stale row created with a slightly different identifier (e.g. created while
        // the commuter was unauthenticated and the id wasn't passed) still gets
        // cleared — otherwise it lingers on the driver map as a phantom waiting
        // marker even though the commuter is already on the ticket manifest.
        if (! empty($data['commuter_id'])) {
            $commuter = Commuter::query()->find($data['commuter_id']);
            BoardingRequest::query()
                ->where('schedule_id', $schedule->id)
                ->where('status', 'waiting')
                ->where(function ($q) use ($data, $commuter) {
                    $q->where('commuter_id', $data['commuter_id']);
                    if ($commuter?->email) {
                        $q->orWhere('commuter_email', $commuter->email);
                    }
                    ...
                    if ($displayName) {
                        $q->orWhere('commuter_name', $displayName);
                    }
                })
    ...
    /**
     * Return the commuter's currently active (non-alighted, non-cancelled) ticket
     * with the fields the commuter app needs to restore trip state after
     * a refresh / logout-login / new device.
     */
    public function activeTicket(Request $request): \Illuminate\Http\JsonResponse
    {
        $data = $request->validate([
            'commuter_id' => ['required', 'integer', 'exists:commuters,id'],
        ]);
        ...
        $ticket = Ticket::query()
            ...
            ->where(function ($q) {
                $q->whereNull('payment_status')
                    ->orWhereNotIn('payment_status', ['cancelled', 'expired']);
            })
            ->orderByDesc('id')
            ->first();

        if (! $ticket) {
            return response()->json(['success' => true, 'ticket' => null]);
        }
        ...
        return response()->json([
            ...
            'ticket' => [
                ...
                'from_stop_index'  => $ticket->from_stop_index,
                'to_stop_index'    => $ticket->to_stop_index,
            ],
        ]);
    }
    ...
    public function alight(Request $request)
    {
        ...
        // Ownership gate: the alight action must come from
        //   (a) the commuter who owns the ticket, or
        //   (b) the driver assigned to the ticket's schedule, or
        //   (c) any caller if the ticket is guest-owned (no commuter_id).
        $ticketOwner = $ticket->commuter_id !== null ? (int) $ticket->commuter_id : null;
        $reqCommuter = isset($data['commuter_id']) ? (int) $data['commuter_id'] : null;
        $reqDriver   = isset($data['driver_id']) ? (int) $data['driver_id'] : null;

        $isGuestTicket    = $ticketOwner === null;
        $isOwner          = $ticketOwner !== null && $reqCommuter === $ticketOwner;
        $isAssignedDriver = false;
        if ($reqDriver !== null) {
            $ticket->loadMissing('schedule');
            $isAssignedDriver = $ticket->schedule && (int) $ticket->schedule->driver_id === $reqDriver;
        }

        if (! $isGuestTicket && ! $isOwner && ! $isAssignedDriver) {
            return response()->json([
                'success' => false,
    ...
    public function liveBuses(Request $request)
    {
        $data = $request->validate([
            ...
            'bus_type' => ['nullable', 'in:regular,aircon'],
            'direction' => ['nullable', 'in:outbound,return'],
        ]);
        ...
        // Direction filter: only return buses heading the way the commuter selected.
        // Prevents a commuter who picked "Cebu→Liloan" from booking a "Liloan→Cebu" bus
        // that happens to share the same route_id but is on the return leg right now.
        if (! empty($data['direction'])) {
            $wantReturn = $data['direction'] === 'return';
            $schedules = $schedules->filter(function (Schedule $s) use ($wantReturn) {
                $isReturn = ($s->trip_leg ?? 1) % 2 === 0;
                return $isReturn === $wantReturn;
            })->values();
        }
        ...
        foreach ($schedules as $s) {
            ...
            $buses[] = [
                'schedule_id' => $s->id,
                'status' => $s->status,
                'leg_status' => $s->leg_status ?? 'pending',
                'trip_leg' => (int) ($s->trip_leg ?? 1),
    ...
    private function estimateBusPosition(Schedule $schedule): ?array
    {
        ...
        $latestPing = DriverLocation::query()
            ->where('schedule_id', $schedule->id)
    ...
    private function findTodaysBookableScheduleForRoute(int $routeId): ?Schedule
    {
        ...
        return Schedule::query()
            ...
            ->orderBy('start_time')
            ->get()
            ->first(fn (Schedule $s) => $this->isScheduleWithinBookableWindow($s));
    }

    /**
     * Today or still-active yesterday trip (end time + 1h grace), aligned with live-buses lookup.
     */
    private function isScheduleWithinBookableWindow(Schedule $schedule): bool
    {
        $today = $this->commuterLocalToday();
        $yesterday = Carbon::now('Asia/Manila')->subDay()->toDateString();
        $date = $this->scheduleDateYmd($schedule);
        ...
        $now = Carbon::now('Asia/Manila');
        [, $windowEnd] = $schedule->windowBounds();

        return $windowEnd->copy()->addHour()->gt($now);
    }
    ...
    public function markPaid(Request $request, string $publicTicketId): \Illuminate\Http\JsonResponse
    {
        ...
        // Ownership gate: only the commuter who owns the ticket may flip payment_status.
        // Guest tickets (no commuter_id) are paid via driver QR scan, not this endpoint.
        $ticketOwner = $ticket->commuter_id !== null ? (int) $ticket->commuter_id : null;
        $reqCommuter = $request->filled('commuter_id') ? (int) $request->input('commuter_id') : null;

        if ($ticketOwner === null || $reqCommuter !== $ticketOwner) {
            return response()->json(['success' => false, 'message' => 'Not your ticket.'], 403);
        }

        // Whitelist methods that may be set via this endpoint — cash flips happen through
        // the driver scan flow, not the commuter app.
        $method = $request->input('payment_method');
        if ($method !== null && ! in_array($method, ['paymaya', 'gcash', 'card'], true)) {
    ...
    public function manifest(int $scheduleId): \Illuminate\Http\JsonResponse
    {
        ...
        $passengers = $schedule->tickets->map(function (Ticket $t) use ($stopsLookup) {
            ...
            $dropStop = $t->to_stop_index !== null ? ($stopsLookup[(int) $t->to_stop_index] ?? null) : null;
            ...
            return [
                ...
                'drop_off_lng' => $dropStop ? (float) ($dropStop['lng'] ?? 0) : null,
                'drop_off_lat' => $dropStop ? (float) ($dropStop['lat'] ?? 0) : null,
                'drop_off_stop_name' => $dropStop['name'] ?? null,
            ];
        })->values();
