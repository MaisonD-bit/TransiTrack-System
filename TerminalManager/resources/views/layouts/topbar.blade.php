...
<style>
    ...
    .notification-item-type.type-route {
        background-color: #e8f5e9;
        color: #1b7f3a;
    }

    .notification-item-type.type-space {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!notificationBellBtn || !notificationDropdown || !closeNotificationDropdown || !notificationBadge) {
        return;
    }
    ...
    function displayNotifications(notifications) {
        ...
        content.innerHTML = notifications.map(notification => {
            ...
            const title = data.subject || typeLabel;
            const message = data.message || 'Notification';
            const createdAt = formatTimeAgo(notification.created_at);
            const url = data.url || '';

            return `
                <div class="notification-item ${unreadClass}" data-id="${escapeHtml(String(notification.id))}" data-url="${escapeHtml(url)}" data-virtual="${notification.is_virtual ? '1' : '0'}">
                    <div class="notification-item-header">
                        <span class="notification-item-title">${escapeHtml(title)}</span>
                        <span class="notification-item-type ${typeClass}">${escapeHtml(typeLabel)}</span>
                    </div>
                    <div class="notification-item-message">${escapeHtml(message)}</div>
                    <div class="notification-item-time"><i class="far fa-clock me-1"></i>${createdAt}</div>
                </div>
            `;
        }).join('');
    ...
    async function handleNotificationClick(item) {
        ...
        const isVirtual = item.dataset.virtual === '1';
    ...
    function getTypeLabel(type) {
        const labels = {
            ...
            route_stop_request: 'Route stops',
            terminal_space_extension_request: 'Space',
        };

        return labels[type] || String(type || 'Notification').replace(/_/g, ' ');
    }

    function getTypeClass(type) {
        if (type === 'route_stop_request') return 'type-route';
        if (type === 'terminal_space_extension_request') return 'type-space';
        return '';
    }
