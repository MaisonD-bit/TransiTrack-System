function tmAlert(message, type = 'info') {
...
function viewOperator(operatorId) {
    ...
    if (!operator) {
        tmAlert('Operator not found', 'warning');
        return;
    }
...
async function activateOperator(operatorId) {
    const operatorName = getOperatorName(operatorId);

    const confirmed = await tmConfirm(
        `Are you sure you want to activate ${operatorName}?`,
        'Activate',
        'Cancel'
    );
    if (!confirmed) return;
    ...
    .then(data => {
        if (data.success) {
            ...
            refreshOpenDetailsModal();
            refreshOperators();
            tmAlert(`${operatorName} has been activated!`, 'success');
        } else {
            tmAlert('Error activating operator: ' + (data.message || 'Unknown error'), 'error');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        tmAlert('Failed to activate operator', 'error');
    });
}
...
function showStatusReasonModal(config) {
    ...
    submit.className = 'btn-modal btn-modal-danger';
...
function submitStatusReason() {
    ...
    .then(data => {
        if (data.success) {
            ...
            tmAlert(`${operatorName} has been deactivated!`, 'success');
        } else {
            tmAlert('Error updating operator: ' + (data.message || 'Unknown error'), 'error');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        tmAlert('Failed to update operator', 'error');
    });
}
